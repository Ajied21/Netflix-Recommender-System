...
class recommend:
    def __init__(self, data, target, test_point, n):
        self.data = data
        self.target = target
    ...
    def fit(self):
        ...
        # Fetch the indices of the n nearest point from the data
        self.indices.extend([index for (val, index) in sorted_li[:self.n]])
        # Fetch the categories from the train data target
        for i in self.indices:
            self.categories.append(self.target[i])
        # Fetch the count for each category from the n value
